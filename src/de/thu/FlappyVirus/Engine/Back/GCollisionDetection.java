...
public class GCollisionDetection {
	...
	public static class CollisionField {
		...
		private void init() {
			...
			for (int i = 0; i < content.size(); i++) {
				GObject object = content.get(i);
				if (object.isMoving() && object.isCollidAble()) {
					...
					if (field != null)
						for (CollisionSubField sub : field.getNeighborFields()) {
							...
							if (other != object && other.isCollidAble()
									&& other.getLocation().distance(object.getLocation()) < (other.getHitRadius()+object.getHitRadius())) {
								
								GEventHandler.call(new ObjectsCollideEvent(0, object, other));
	...
	public static abstract class CollisionSubField {
		 ...
		 * @param invert	If invert, then the next fields are zoomed in, otherwise zoomed out
		 */
		...
		/**
		 * The method looks if a field has a direct neighbor. 
		 * A direct neighbor in the corresponding direction has 
		 * a field if it does not have to leave the parent field 
		 * to get to it
		 * 
		...
		/**
		 * Determine the middle of two numbers and 
